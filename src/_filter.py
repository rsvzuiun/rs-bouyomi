...
class FieldChat(FilterMeta):
    name = 'FIELD'
    regex = rb'\x00\(\x11\xCD\xCD[\x01|\x02]\x00\x00\x00.\x00X\x11\x00\x00.\x00\x00\x00([^\x00]+)\x00+([^\x00]+)'
    talk = False

class PartyChat(FilterMeta):
    name = 'PARTY'
    regex = rb'.\x00\(\x11\xCD\xCD.\x00\x00\x00(?:.)+\x00\x58\x11\xCC\xCC\xCC..\x80([^\x00]+)\x00+([^\x00]+)'
    talk = True
...
class WhisperFromTo(FilterMeta):
    name = 'WHISP_FROMTO'
    regex = rb'\x00\(\x11\xCD\xCD.\x00\x00\x00\x9A\x00v\x11\x00\x00\x00\x00(.{18})([^\x00]+).*\x00X\x11\xCC\xCC..L\x80'

class WhisperTo(FilterMeta):
    name = 'WHISP_TO'
    regex = rb'\x00\(\x11\xCD\xCD.\x00\x00\x00\x9A\x00v\x11\x00\x00\x00\x00(.{18})([^\x00]+)'

class WhisperFrom(FilterMeta):
    name = 'WHISP_FROM'
    regex = rb'\x00\(\x11\xCD\xCD\x01\x00\x00\x00.\x00X\x11\xCC\xCC..L\x80([^\x00]+)\x00+([^\x00]+)'
